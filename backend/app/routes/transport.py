...
async def import_transport_excel(
    ...
    for sheet_name in xls.sheet_names:
        ...
        if not clean_sheet_name or clean_sheet_name.lower().startswith(
            ("sheet", "summary", "template")
        ):
            continue
        ...
        for col in df.columns:
            ...
            if (
                ...
                "SHIPPER" in col_up
                or "DESCRIPTION" in col_up
                or "فرستنده" in col_up
                or "تفصیلات" in col_up
            ):
        ...
        client = (
            db.query(models.ExportClient)
            .filter(models.ExportClient.name == clean_sheet_name)
            .first()
        )
        if not client:
        ...
        for _, row in df.iterrows():
            ...
            shipper_desc = (
                str(row.get("shipper_description", "")).strip()
                if pd.notna(row.get("shipper_description"))
                else ""
            )
            bl_no = (
                str(row.get("bill_of_lading", "")).strip()
                if pd.notna(row.get("bill_of_lading"))
                else ""
            )
            inv_no = (
                str(row.get("invoice_no", "")).strip()
                if pd.notna(row.get("invoice_no"))
                else ""
            )
            cont_no = (
                str(row.get("container_no", "")).strip()
                if pd.notna(row.get("container_no"))
                else ""
            )
            ...
            ledger = models.TransportLedger(
                ...
                consignee=(
                    str(row.get("consignee", "")).strip()
                    if pd.notna(row.get("consignee"))
                    else ""
                ),
...
def create_export_client(
    ...
    existing = (
        db.query(models.ExportClient)
        .filter(models.ExportClient.name == payload.name)
        .first()
    )
    if existing:
        raise HTTPException(
...
@router.get(
    "/clients/{client_id}/ledger", response_model=List[schemas.TransportLedgerResponse]
)
def get_client_ledger(
    ...
    client = (
        db.query(models.ExportClient)
        .filter(models.ExportClient.id == client_id)
        .first()
    )
    if not client:
...
def get_client_ledger_paginated(
    ...
    query = db.query(models.TransportLedger).filter(
        models.TransportLedger.client_id == client_id
    )
    if search:
        s = f"%{search.strip()}%"
        query = query.filter(
    ...
    ledgers = (
        query.order_by(models.TransportLedger.id.asc())
        .offset(offset)
        .limit(page_size)
        .all()
    )
...
def create_transport_transaction(
    ...
    if payload.client_id:
        client = (
            db.query(models.ExportClient)
            .filter(models.ExportClient.id == payload.client_id)
            .first()
        )
    ...
    if not client and payload.client_name:
        client = (
            db.query(models.ExportClient)
            .filter(models.ExportClient.name == payload.client_name)
            .first()
        )
        if not client:
            client = models.ExportClient(
                name=payload.client_name,
                contact_info="Transport Client",
                currency="USD",
            )
            db.add(client)
            db.commit()
    ...
    if not client:
        ...
        if not client:
            client = models.ExportClient(
                name="NAJEB-AMIN LTD", contact_info="Kandahar, AF", currency="USD"
            )
            db.add(client)
            db.commit()
            db.refresh(client)
...
def delete_transport_transaction(
    ...
    ledger = (
        db.query(models.TransportLedger)
        .filter(models.TransportLedger.id == transaction_id)
        .first()
    )
    if not ledger:
...
def get_transport_grand_summary(
    ...
    total_containers = sum(
        l.quantity for l in all_ledgers if l.transaction_type == "shipment"
    )
