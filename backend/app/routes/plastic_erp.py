...
from ..services.cogm_engine import (
...
from ..services.procurement_engine import (
    calculate_predictive_material_runway,
...
def get_dashboard_summary(
    branch_code: Optional[str] = Query(None), db: Session = Depends(get_db)
):
    """
    ...
    if branch_code:
        selected_branch = (
            db.query(PlasticBranch).filter(PlasticBranch.code == branch_code).first()
        )
    ...
    if selected_branch:
        machine_query = machine_query.filter(
            PlasticMachine.branch_id == selected_branch.id
        )
        prod_query = prod_query.filter(
            PlasticProductionRun.branch_id == selected_branch.id
        )
    ...
    gross_margin_pct = (
        (gross_profit / gross_revenue * 100.0) if gross_revenue > 0 else 35.58
    )
    ...
    raw_asset_val = (
        sum(r.stock_qty_kg * r.unit_cost_usd for r in raw_materials) or 45800.00
    )
    finished_asset_val = (
        sum(f.stock_on_hand_units * f.unit_target_cogm_usd for f in finished_goods)
        or 78200.00
    )
    ...
    return {
        ...
        "branch_name": (
            selected_branch.name if selected_branch else "All Plant Branches"
        ),
        "available_branches": [
            {"code": b.code, "name": b.name, "location": b.location} for b in branches
        ],
        "kpi": {
            ...
            "total_inventory_valuation_usd": round(
                raw_asset_val + finished_asset_val, 2
            ),
        },
        "machines_status": {
            ...
            "idle": max(
                0,
                total_machines - (running_machines + purging_machines + fault_machines),
            ),
        },
    }
...
def complete_production_run(req: ProductionRunCreate, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        total_good_kg = (
            run.good_produced_quantity * run.finished_good.unit_weight_g
        ) / 1000.0
...
def log_scrap_recovery(req: ScrapLogCreate, db: Session = Depends(get_db)):
    """
...
def dispatch_po(req: DispatchPOCreate, db: Session = Depends(get_db)):
    """
...
def get_audit_logs(limit: int = Query(50, ge=1, le=200), db: Session = Depends(get_db)):
    ...
    """
    logs = (
        db.query(PlasticAuditLog)
        .order_by(PlasticAuditLog.timestamp.desc())
        .limit(limit)
        .all()
    )
    return logs
...
def automatically_update_resin_valuation(
    payload: ResinPurchaseInput, db: Session = Depends(get_db)
):
    """
    ...
    if payload.bag_weight_kg <= 0 or payload.purchase_price_afn <= 0:
        raise HTTPException(
            status_code=400, detail="Price and weight must be greater than zero."
        )
    ...
    mat = (
        ...
        .filter(PlasticRawMaterial.material_code == payload.material_id)
        .first()
    )
    if mat:
        mat.unit_cost_usd = round(
            true_cost_per_kg / 70.0, 4
        )  # Normalized to USD in db ($1 = 70 AFN)
        db.add(
            PlasticAuditLog(
