...
from ..auth_dependencies import create_access_token, require_authenticated_request, require_administrator_request
...
def setup_owner(payload: schemas.SetupRequest, db: Session = Depends(get_db)):
    ...
    access_token = create_access_token(
        data={"sub": user.username, "role": user.role, "assigned_group_id": user.assigned_group_id, "assigned_branch_id": user.assigned_branch_id},
        expires_delta=timedelta(days=1)
    )
    audit(db, "setup_admin", "success", user.username, user.id, "Owner setup completed")
    db.commit()
    return {"token": access_token, "expires_at": now + timedelta(days=1), "user": public_user(user), "must_change_password": False}
...
def login(payload: schemas.LoginRequest, db: Session = Depends(get_db)):
    ...
    db.commit()
    db.refresh(user)
    return {"token": access_token, "expires_at": now + expires_delta, "user": public_user(user), "must_change_password": user.must_change_password}
...
@router.post("/logout")
def logout(user: models.User = Depends(require_authenticated_request), db: Session = Depends(get_db)):
...
def list_users(admin: models.User = Depends(require_administrator_request), db: Session = Depends(get_db)):
...
def create_user(payload: schemas.UserCreate, admin: models.User = Depends(require_administrator_request), db: Session = Depends(get_db)):
...
def update_user(user_id: int, payload: schemas.UserUpdate, admin: models.User = Depends(require_administrator_request), db: Session = Depends(get_db)):
...
def delete_user(user_id: int, admin: models.User = Depends(require_administrator_request), db: Session = Depends(get_db)):
    if admin.id == user_id:
