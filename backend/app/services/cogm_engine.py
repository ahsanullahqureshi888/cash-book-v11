...
def calculate_bom_cost_preview(
    ...
    blended_resin_cost_per_kg = (virgin_ratio * virgin_resin_price_per_kg) + (
        regrind_ratio * regrind_price_per_kg
    )
    ...
    total_hourly_machine_burn = (
        power_cost_per_hour + hourly_overhead_rate + operator_hourly_wage
    )
    ...
    gross_margin = (
        ((suggested_price - total_unit_cogm) / suggested_price * 100.0)
...
def execute_production_run_and_post_ledger(
    ...
    finished_good = (
        db.query(PlasticFinishedGood).filter(PlasticFinishedGood.sku == sku).first()
    )
    ...
    total_raw_material_used_kg = (
        (good_produced_quantity + scrap_quantity_units)
        * unit_weight_kg
        * scrap_allowance_multiplier
    )
    ...
    if raw_mat:
        raw_mat.stock_qty_kg = max(
            0.0, raw_mat.stock_qty_kg - total_raw_material_used_kg
        )
    ...
    overhead_cost = (
        machine_hours_logged * machine.hourly_overhead_rate_usd
    ) + power_cost
    ...
    total_cogm = (
        direct_material_cost + direct_labor_cost + overhead_cost - scrap_salvage_credit
    )
    unit_cogm = (
        (total_cogm / good_produced_quantity) if good_produced_quantity > 0 else 0.0
    )
