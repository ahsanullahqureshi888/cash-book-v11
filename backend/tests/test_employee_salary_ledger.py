...
def create_test_employee(db, name="Test Emp", salary=30000.0, joining_date=None, currency="AFN", end_date=None):
...
def test_case_1_joining_date_present_proration(db):
    ...
    emp = create_test_employee(db, name="Emp Case 1", salary=31000.0, joining_date=date(2026, 7, 16))
    
    # Ledger through July 2026
    ledger = payroll.calculate_employee_salary_ledger(db, emp.id, to_date=date(2026, 7, 31))
...
def test_case_3_no_joining_date_partial_payment(db):
    ...
    payroll.create_salary_payment(db, schemas.SalaryPaymentCreate(
        employee_id=emp.id,
        month=today.month,
        year=today.year,
        amount=10000.0,
...
def test_case_4_joining_date_historical_unpaid_salary(db):
    ...
    emp = create_test_employee(db, name="Emp Case 4", salary=15000.0, joining_date=date(2026, 1, 1))
    ...
    ledger = payroll.calculate_employee_salary_ledger(db, emp.id, to_date=date(2026, 2, 28))
...
def test_case_5_future_joining_date(db):
    ...
    emp = create_test_employee(db, name="Emp Case 5", salary=30000.0, joining_date=date(2099, 1, 1))
    
    ledger = payroll.calculate_employee_salary_ledger(db, emp.id, to_date=date(2026, 7, 20))
...
def test_case_6_salary_rate_change(db):
    ...
    emp = create_test_employee(db, name="Emp Case 6", salary=10000.0, joining_date=date(2026, 1, 1))
    
    # Record salary history change effective March 1, 2026
    payroll.create_salary_history(db, emp, schemas.SalaryHistoryCreate(
        new_salary=15000.0,
        new_currency="AFN",
    ...
    ledger = payroll.calculate_employee_salary_ledger(db, emp.id, to_date=date(2026, 4, 30))
...
def test_case_7_terminated_employee(db):
    ...
    ledger = payroll.calculate_employee_salary_ledger(db, emp.id, to_date=date(2026, 5, 31))
    accruals = [e for e in ledger["entries"] if e["entry_type"] == "salary_accrual"]
...
def test_case_8_partial_multiple_payments_cashbook(db):
    emp = create_test_employee(db, name="Emp Case 8", salary=20000.0, joining_date=date(2026, 1, 1))
    
    p1 = payroll.create_salary_payment(db, schemas.SalaryPaymentCreate(
        employee_id=emp.id, month=1, year=2026, amount=5000.0, payment_date=date(2026, 1, 15)
    ))
    p2 = payroll.create_salary_payment(db, schemas.SalaryPaymentCreate(
        employee_id=emp.id, month=1, year=2026, amount=10000.0, payment_date=date(2026, 1, 20)
    ))
    
    ledger = payroll.calculate_employee_salary_ledger(db, emp.id, to_date=date(2026, 1, 31))
...
def test_case_9_multiple_currencies(db):
    emp_afn = create_test_employee(db, name="Emp AFN", salary=20000.0, joining_date=date(2026, 1, 1), currency="AFN")
    emp_usd = create_test_employee(db, name="Emp USD", salary=500.0, joining_date=date(2026, 1, 1), currency="USD")
    
    ledger_afn = payroll.calculate_employee_salary_ledger(db, emp_afn.id, to_date=date(2026, 1, 31))
    ledger_usd = payroll.calculate_employee_salary_ledger(db, emp_usd.id, to_date=date(2026, 1, 31))
...
def test_case_10_backup_restore_ledger_equivalence(db):
    emp = create_test_employee(db, name="Emp Case 10", salary=25000.0, joining_date=date(2026, 1, 1))
    payroll.create_salary_payment(db, schemas.SalaryPaymentCreate(
        employee_id=emp.id, month=1, year=2026, amount=10000.0, payment_date=date(2026, 1, 20)
    ))
    payroll.create_salary_adjustment(db, emp.id, schemas.EmployeeSalaryAdjustmentCreate(
        date=date(2026, 1, 25), period="2026-01", amount=2000.0, adjustment_type="bonus", reason="Performance bonus"
    ))
    
    ledger_before = payroll.calculate_employee_salary_ledger(db, emp.id, to_date=date(2026, 1, 31))
    ...
    emp2 = db2.query(models.Employee).filter(models.Employee.full_name == "Emp Case 10").first()
    
    ledger_after = payroll.calculate_employee_salary_ledger(db2, emp2.id, to_date=date(2026, 1, 31))
    
    assert ledger_before["summary"] == ledger_after["summary"]
    assert len(ledger_before["entries"]) == len(ledger_after["entries"])
    assert ledger_before["entries"][0]["running_balance"] == ledger_after["entries"][0]["running_balance"]
