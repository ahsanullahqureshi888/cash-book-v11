...
def seed_groups_and_branches():
    ...
    try:
        ...
        if db.query(models.Group).count() > 0:
            print(
                "Groups and branches already exist. Checking transaction branch assignments..."
            )
            unlinked_txs = (
                db.query(models.Transaction)
                .filter(models.Transaction.branch_id == None)
                .all()
            )
            if unlinked_txs:
                print(
                    f"Linking {len(unlinked_txs)} unlinked transactions to branches..."
                )
                kabul = (
                    db.query(models.Branch)
                    .filter(models.Branch.name == "Kabul")
                    .first()
                )
                kandahar_a = (
                    db.query(models.Branch)
                    .filter(models.Branch.name == "Kandahar")
                    .first()
                )
                herat = (
                    db.query(models.Branch)
                    .filter(models.Branch.name == "Herat")
                    .first()
                )
                branch_ids = (
                    [kabul.id, kandahar_a.id, herat.id]
                    if (kabul and kandahar_a and herat)
                    else []
                )
                if branch_ids:
        ...
        consolidated_b = models.Branch(
            name="All Branches (Consolidated)", group_id=group_b.id
        )
        ...
        print(
            f"Created Group A: Branch {kabul.name} (ID: {kabul.id}), Branch {kandahar_a.name} (ID: {kandahar_a.id})"
        )
        print(
            f"Created Group B: Branch {herat.name} (ID: {herat.id}), Branch {kandahar_b.name} (ID: {kandahar_b.id}), Branch {consolidated_b.name} (ID: {consolidated_b.id})"
        )
