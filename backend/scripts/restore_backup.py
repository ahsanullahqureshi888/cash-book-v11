                    ...
                    if hasattr(column.default, "arg") and not callable(
                        ...
                        if isinstance(default_val, str):
                            ...
                            default_clause = (
                                ...
                                else " DEFAULT " + ("1" if default_val else "0")
                            )
                        else:
                            default_clause = f" DEFAULT {default_val}"
...
def main():
    parser = argparse.ArgumentParser(
        description="Seed and recover the cash book database from JSON backup."
    )
    parser.add_argument(
        "--backup-file",
    ...
    for emp in employees_data:
        if emp["account_id"] not in acc_ids:
            raise ValueError(
                f"Constraint Violation: Employee ID {emp['id']} references missing Account ID {emp['account_id']}."
            )
    ...
    for tx in transactions_data:
        if tx.get("account_id") and tx["account_id"] not in acc_ids:
            raise ValueError(
                f"Constraint Violation: Transaction ID {tx['id']} references missing Account ID {tx['account_id']}."
            )
        if tx.get("employee_id") and tx["employee_id"] not in emp_ids:
            raise ValueError(
                f"Constraint Violation: Transaction ID {tx['id']} references missing Employee ID {tx['employee_id']}."
            )
    ...
    for hist in history_data:
        if hist["employee_id"] not in emp_ids:
            raise ValueError(
                f"Constraint Violation: Salary History ID {hist.get('id')} references missing Employee ID {hist['employee_id']}."
            )
    ...
    for pay in payments_data:
        if pay["employee_id"] not in emp_ids:
            raise ValueError(
                f"Constraint Violation: Salary Payment ID {pay.get('id')} references missing Employee ID {pay['employee_id']}."
            )
        if pay.get("cashbook_entry_id") and pay["cashbook_entry_id"] not in tx_ids:
            raise ValueError(
                f"Constraint Violation: Salary Payment ID {pay.get('id')} references missing Transaction ID {pay['cashbook_entry_id']}."
            )
    ...
    if not args.yes:
        confirm = input(
            "WARNING: This will CLEAR existing settings, accounts, employees, transactions, and salary payments in the target database. Proceed? (y/N): "
        )
    ...
    try:
        ...
        setting_row = models.Setting(
            ...
            company_name=settings_data.get(
                "company_name", "BAWAR STAR PLASTIC INDUSTRY"
            ),
            ...
            default_exchange_rate=float(
                settings_data.get("default_exchange_rate", 64.3)
            ),
            ...
            print_footer_text=settings_data.get(
                "print_footer_text", "Prepared by BAWAR STAR PLASTIC INDUSTRY"
            ),
            auto_logout_minutes=int(settings_data.get("auto_logout_minutes", 30)),
            created_at=parse_datetime(settings_data.get("created_at"))
            or datetime.utcnow(),
            updated_at=parse_datetime(settings_data.get("updated_at"))
            or datetime.utcnow(),
        )
        ...
        for acc in accounts_data:
            session.add(
                models.Account(
                    ...
                    note=acc.get("note", ""),
                    created_at=parse_datetime(acc.get("created_at"))
                    or datetime.utcnow(),
                    updated_at=parse_datetime(acc.get("updated_at"))
                    or datetime.utcnow(),
                )
            )
        ...
        for emp in employees_data:
            session.add(
                models.Employee(
                    ...
                    notes=emp.get("notes", ""),
                    created_at=parse_datetime(emp.get("created_at"))
                    or datetime.utcnow(),
                    updated_at=parse_datetime(emp.get("updated_at"))
                    or datetime.utcnow(),
                )
            )
        ...
        for tx in transactions_data:
            session.add(
                models.Transaction(
                    ...
                    or datetime.utcnow(),
                    updated_at=parse_datetime(tx.get("updated_at"))
                    or datetime.utcnow(),
                )
            )
        ...
        for hist in history_data:
            session.add(
                models.SalaryHistory(
                    ...
                    changed_at=parse_datetime(hist.get("changed_at"))
                    or datetime.utcnow(),
        ...
        for pay in payments_data:
            session.add(
                models.SalaryPayment(
                    ...
                    created_at=parse_datetime(pay.get("created_at"))
                    or datetime.utcnow(),
                    updated_at=parse_datetime(pay.get("updated_at"))
                    or datetime.utcnow(),
                )
            )
