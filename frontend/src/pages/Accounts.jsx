...
export default function Accounts({ 
  ...
  const renderMobileCard = (row) => {
    ...
    const cleanAddress = unescapeText(row.address);
    return (
      <div key={row.id} className="account-mobile-card p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 space-y-3 shadow-xs">
        <div className="account-mobile-card__header flex items-center justify-between gap-2">
          <div>
            <strong className="account-mobile-card__name text-sm font-bold text-slate-900 dark:text-white">{cleanName}</strong>
            {cleanAddress && <p className="account-mobile-card__address text-xs text-slate-400">{cleanAddress}</p>}
          </div>
          <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase ${getTypeBadgeClass(row.account_type)}`}>
            {row.account_type}
          </span>
        </div>

        <div className="account-mobile-card__body space-y-1 text-xs font-mono">
          <div className="flex items-center justify-between">
            <span className="text-slate-500 font-sans">{t('Phone')}:</span>
            <strong className="mono-text">{row.phone || '-'}</strong>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-slate-500 font-sans">{t('Opening AFN')}:</span>
            <strong className="mono-text text-emerald-600">{currency(row.opening_balance_afn || 0)}</strong>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-slate-500 font-sans">{t('Opening USD')}:</span>
            <strong className="mono-text text-slate-700">{currency(row.opening_balance_usd || 0, 'USD')}</strong>
          </div>
        </div>

        <div className="account-mobile-card__footer flex items-center gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
          <NavLink to={`/ledger?account=${row.id}`} className="px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-xs font-bold flex items-center gap-1.5 text-slate-700 dark:text-slate-200">
            <ScrollText size={15} />
            <span>{t('Ledger')}</span>
          </NavLink>
          <button type="button" className="px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-xs font-bold flex items-center gap-1.5 text-slate-700 dark:text-slate-200" onClick={() => onEdit(row)}>
            <Edit size={15} />
            <span>{t('Edit')}</span>
          </button>
          <button type="button" className="px-3 py-1.5 rounded-lg bg-rose-50 dark:bg-rose-950/40 text-xs font-bold flex items-center gap-1.5 text-rose-600 dark:text-rose-400" onClick={() => onDelete(row)}>
            <Trash2 size={15} />
            <span>{t('Delete')}</span>
          </button>
        </div>
