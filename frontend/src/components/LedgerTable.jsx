...
export default function LedgerTable({ rows, dateDisplayFormat, onReceipt }) {
  ...
  const columns = useMemo(() => [
    ...
    { key: 'cash_in_afn', label: 'Cash In', render: (row) => <span className="font-mono text-emerald-600 dark:text-emerald-400 font-semibold">{row.cash_in_afn ? currency(row.cash_in_afn) : '-'}</span>, className: 'w-24 text-right' },
    { key: 'cash_out_afn', label: 'Cash Out', render: (row) => <span className="font-mono text-rose-600 dark:text-rose-400 font-semibold">{row.cash_out_afn ? currency(row.cash_out_afn) : '-'}</span>, className: 'w-24 text-right' },
    { key: 'balance', label: 'Balance', render: (row) => <span className={`font-mono font-bold ${row.balance >= 0 ? 'text-emerald-700 dark:text-emerald-300' : 'text-rose-700 dark:text-rose-300'}`}>{currency(row.balance)}</span>, className: 'w-28 text-right' },
